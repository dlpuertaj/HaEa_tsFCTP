    ...
    void testReturnProduct() {
        ...
        for (int j = 0; j < networks[0].J; j++) {
            if (networks[0].distributionInbound[j] > 0) {
                dcWithInbound = j;
                System.out.println("\nUsing dc..." + dcWithInbound);
                originalInbound = networks[0].distributionInbound[dcWithInbound];
                break;
            }
        }
        networks[0].distributionInbound[dcWithInbound] += 50;

        Distributor.returnProduct(dcWithInbound, 50, networks[0]);
        assertEquals(originalInbound, networks[0].distributionInbound[dcWithInbound]);//TODO fix fail
